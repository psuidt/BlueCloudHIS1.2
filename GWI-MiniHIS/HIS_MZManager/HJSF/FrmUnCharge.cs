...
namespace HIS_MZManager.HJSF
{
    public partial class FrmUnCharge : GWI.HIS.Windows.Controls.BaseForm
    {
        ...
        /// </summary>
        /// <returns></returns>
        private Prescription[] GetPrescriptionRemanentFromGrid( bool allPrescription ,out List<int> returnedDocPrescIdList )
        {
            returnedDocPrescIdList = new List<int>();
            ...
            for ( presIndex = 0 ; presIndex < prescCount ; presIndex++ )
            {
                ...
                for ( int i = start ; i <= end ; i++ )
                {
                    ...
                    {
                        noreturnValue = false;
                        returnedDocPrescIdList.Add( Convert.ToInt32( dgvPresc[DocPresId.Name, i].Value ) );
                    }
        ...
        private void btnOK_Click( object sender , EventArgs e )
        {
            ...
            string invoiceNo = txtPerfChar.Text.Trim() + txtInvoiceNo.Text;
            if ( invoiceNo != lblInvoiceNo.Text )
            {
                MessageBox.Show( "输入的发票号和当前已检索的发票号不一致，请确认！" );
                return;
            }

            if ( dgvPresc.Rows.Count == 0 )
            {
                MessageBox.Show( "没有可操作的内容，请输入正确的发票号后按回车[Enter]键读取要操作的退费信息" , "" , MessageBoxButtons.OK , MessageBoxIcon.Error );
                return;
            }
        ...
        private void FrmUnCharge_Load( object sender , EventArgs e )
        {
            ...
            txtInvoiceNo.MaxLength = Convert.ToInt32( HIS.MZ_BLL.OPDParamter.Parameters["015"] );
            this.Width = Screen.PrimaryScreen.WorkingArea.Width;
            this.Left = 0;

            dtbChargeItems = HIS.MZ_BLL.PublicDataReader.GetItemSelectedCardDataSource( 1 );
            DataTable tbPerfChar = HIS.MZ_BLL.InvoiceManager.Get_Invoice_PerfChar();
        ...
        private void dgvPresc_CellEndEdit( object sender , DataGridViewCellEventArgs e )
        {
            ...
            try
            {
                    ...
                    || e.ColumnIndex == dgvPresc.Columns["PresAmount"].Index )
                {
                    ...
                    decimal packnumOld = (dgvPresc["PACK_NUM", rowIndex].Value == null || dgvPresc["PACK_NUM", rowIndex].Value.ToString().Trim() == "") ? 0 : Convert.ToDecimal( dgvPresc["PACK_NUM", rowIndex].Value );
                    decimal basenumOld = (dgvPresc["BASE_NUM" , rowIndex].Value == null || dgvPresc["BASE_NUM" , rowIndex].Value.ToString().Trim() == "") ? 0 : Convert.ToDecimal( dgvPresc["BASE_NUM" , rowIndex].Value );
                    decimal packnumRet = (dgvPresc["ReturnPackNum" , rowIndex].Value == null || dgvPresc["ReturnPackNum" , rowIndex].Value.ToString().Trim() == "" ) ? 0 : Convert.ToDecimal( dgvPresc["ReturnPackNum" , rowIndex].Value );
                    decimal basenumRet = ( dgvPresc["ReturnBaseNum", rowIndex].Value == null || dgvPresc["ReturnBaseNum", rowIndex].Value.ToString().Trim() == "" ) ? 0 : Convert.ToDecimal( dgvPresc["ReturnBaseNum", rowIndex].Value );


                    decimal packnum = packnumOld - packnumRet;
                    decimal basenum = basenumOld - basenumRet;
                    decimal presAmount = ( dgvPresc["PresAmount", rowIndex].Value == null || dgvPresc["PresAmount", rowIndex].Value.ToString().Trim()=="" ) ? 1 : Convert.ToDecimal( dgvPresc["PresAmount", rowIndex].Value );
                    //计算本张处方金额
                    decimal presTotal = 0;
        ...
        private void txtInvoiceNo_Leave( object sender , EventArgs e )
        {
            ...
            //try
            //{
            //    currentPatient = new HIS.MZ_BLL.OutPatient( txtInvoiceNo.Text , OPDBillKind.门诊收费发票 );

            //    txtPatientName.Text = currentPatient.PatientName;
            //    lblFeeType.Text = PublicDataReader.GetPatientTypeNameByCode( currentPatient.MediType );

            //    currentInvoice = new HIS.MZ_BLL.Invoice( txtPerfChar.Text.Trim(), txtInvoiceNo.Text , OPDBillKind.门诊收费发票 );
            //    if ( currentInvoice.RecordFlag != 0 )
            //    {
            //        MessageBox.Show( "该发票已经退费，请确认输入的发票号是否正确！" );
            //        txtInvoiceNo.Focus( );
