...
private void button2_Click( object sender, EventArgs e )
{
    ...
    columns[0].ColumnName = "旧存";
    columns[0].ColumnField = "V1";

    columns[1].ColumnName = "新收";
    columns[1].ColumnField = "V2";

    columns[2].ColumnName = "结案";
    columns[2].ColumnField = "V3";

    columns[3].ColumnName = "存案";
        ...
        public override void DrawGraph()
        {
            ...
            if (base.XAxisScaleRefrence == DataTableStruct.Rows)
            {
                ...
                for (int i = 0; i < base.DataSource.Rows.Count; i++)
                {
                    ...
                    labels[i] = base.DataSource.Rows[i][base.CNNameColumn].ToString() + "("+values[i].ToString()+")";
                }
            ...
            else
            {
                ...
                for (int i = 0; i < base.ShowValueColumns.Length; i++)
                {
                    ...
                    total += values[i];
                    labels[i] = base.ShowValueColumns[i].ColumnName + "(" + values[i].ToString() + ")";
                }
