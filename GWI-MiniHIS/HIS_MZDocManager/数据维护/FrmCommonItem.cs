...
{
    public partial class FrmCommonItem : GWI.HIS.Windows.Controls.BaseMainForm,Action.IDataInstall
    {
        ...
        private HIS.MZDoc_BLL.CommonItem _commonItem;

        private int _rowIndex = -1;
        private int _firstInputColumnIndex = 1;

        public FrmCommonItem(long currentUserId, long currentDeptId, string chineseName)
        {
            InitializeComponent();
            ...
            _commonItem = new HIS.MZDoc_BLL.CommonItem();
            dGVEMain.SetSelectionCardDataSource(_commonItem.LoadShowCardData(), _firstInputColumnIndex);
        ...
        public HIS.MZDoc_BLL.IBaseCommon CommonSub
        {
            get
            {
                ...
                {
                    _commonItem = (HIS.MZDoc_BLL.CommonItem)HIS.SYSTEM.PubicBaseClasses.ApiFunction.DataTableToObject((DataTable)this.dGVEMain.DataSource, _rowIndex, _commonItem);
                    _commonItem.Record_Doc = (int)_currentUser.EmployeeID;
                }
                return _commonItem;
            }
            set
            {
                _commonItem = (HIS.MZDoc_BLL.CommonItem)value;
                _commonItem.Record_Doc = (int)_currentUser.EmployeeID;
            }
        }
        ...
        private void dGVEMain_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            {
                DataGridViewReadOnly = true;
            }
        }
        #endregion       

        private void FrmCommonItem_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyData)
            {
                case Keys.F2:	//保存
