...
namespace HIS_ReportManager
{
    public partial class FrmSetLocation : BaseForm
    {
        ...
        private List<Paramater> locationSettings;
        /// <summary>
        /// 
        /// </summary>
        public List<Paramater> LocationSettings
        {
            get
            {
        ...
        void ctrl_SizeChanged(object sender, EventArgs e)
        {
            ...
            if (obj != null)
            {
                ((Paramater)obj).WIDTH = ((Control)sender).Width;
                ((Paramater)obj).HEIGHT = ((Control)sender).Height;
            }
        }
        ...
        void sourceCtrl_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (beginMove)
            {
                ...
                if (obj != null)
                {
                    ((Paramater)obj).UIC_X_LOCA = p.X;
                    ((Paramater)obj).UIC_Y_LOCA = p.Y;
                }
                if (p.X > 0 && p.Y > 0)
                {
        ...
        private List<Paramater> lstTableFields;
        /// <summary>
        /// 
        /// </summary>
        /// <param name="LstTableFields"></param>
        public FrmSetLocation(List<Paramater> LstTableFields)
        {
            InitializeComponent();
        ...
        private void FrmSetLocation_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < lstTableFields.Count; i++)
            {
                ...
                ctrl.Name = lstTableFields[i].PARAMETER;

                if (lstTableFields[i].UIC_TYPE == 2)
                {
                    ctrl = new CheckBox();
                    ((CheckBox)ctrl).Text = lstTableFields[i].PARAMETER ;
                    ((CheckBox)ctrl).Location = new Point(lstTableFields[i].UIC_X_LOCA, lstTableFields[i].UIC_Y_LOCA);
                ...
                {
                    ctrl = new TextBox();
                    ((TextBox)ctrl).Text = lstTableFields[i].PARAMETER ;
                    ((TextBox)ctrl).Location = new Point(lstTableFields[i].UIC_X_LOCA, lstTableFields[i].UIC_Y_LOCA);
