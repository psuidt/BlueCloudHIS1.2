...
namespace HIS_ZYManager
{
    public partial class ZYBaseFrom : GWI.HIS.Windows.Controls.BaseMainForm
    {
        ...
        //刷新
        public void llabrush_LinkClicked(object sender, EventArgs e)
        {
        ...
        private void tbInpatNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                if (zypatlist != null)
                {
                    List<ZY_PatList> patlist = zypatlist.FindAll(x => x.CureNo == this.tbInpatNo1.Text);
        ...
        private void tbPatName_TextChanged(object sender, EventArgs e)
        {
            if (zypatlist != null)
            {
                List<ZY_PatList> patlist = zypatlist.FindAll(x => (x.patientInfo.PYM.ToLower().Contains(this.tbPatName1.Text.Trim()) || x.patientInfo.PatName.Contains(this.tbPatName1.Text.Trim())));
