        ...
        public override string DataSQL
        {
            get
            {
                ...
                if ( StatType != StatClassType.收费支付类型 )
                {
                    ...
                    sbSQL.Append( " from " );
                    sbSQL.Append( " (select costmasterid,chargecode,costdate,record_flag,accountid from mz_costmaster) a," );
                ...
                else
                {
                    ...
                    sbSQL.Append( " group by chargecode" );
                    sbSQL.Append( " union all " );
                    sbSQL.Append( "select chargecode as ID,'医保农合记账' as code, sum(village_fee) as total_fee " );
                    sbSQL.Append( " from (" + base.BaseSQL2 + ") a " );
                    sbSQL.Append( " group by chargecode" );
                    sbSQL.Append( " union all " );
