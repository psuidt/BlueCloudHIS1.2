...
namespace HIS.YP_BLL
{
    ...
    /// 退药台帐处理器
    /// </summary>
    class RefundAccount : AccountWriter
    {
        /// <summary>
        /// 根据单据信息构造台帐信息
        /// </summary>
           ...
           int accountYear, int accountMonth, int smallUnit)
        {
            ...
            //退药的零售金额是根据发药的零售金额按比例计算出来，不知可否？
            //account.LenderFee = order.RetailFee * master.RecipeNum;
            //account.LenderNum = order.DrugOCNum * master.RecipeNum;
            ...
            else
            {
                ...
                account.LenderNum = order.DrugOCNum;
            }
        ...
        public override void WriteAccount(BillMaster billMaster, List<BillOrder> orderList, Hashtable storeTable)
        {
            try
            {
                ...
                {
                    string queryKey = order.MakerDicID.ToString() + order.OrderRecipeID.ToString();
                    storeNum = ((YP_StoreNum)(storeTable[queryKey])).storeNum;
                    smallUnit = ((YP_StoreNum)(storeTable[queryKey])).smallUnit;
